				...
				document.addEventListener('DOMContentLoaded',
					function() {
								...
								.addEventListener(
										...
										function() {
											...
											exerciseContainer.innerHTML = '<div class="col-6">'
													...
													+ '<div class="col-2">'
													+ '<label for="inputAddress" class="form-label">Broj serija</label>'
													+ '<form:input type="number" class="form-control" id="inputAddressSerije" path="serije" />'
													+ '</div>'
													+ '<p>x</p>'
													+ '<div class="col-2">'
													+ '<label for="inputAddress" class="form-label">Broj ponavljanja</label>'
													+ '<form:input type="number" class="form-control" id="inputAddressPonavljanja" path="ponavljanja" />'
													+ '</div>';
						...
						removeButton.addEventListener('click',
							function(event) {
								var exerciseContainer = document.getElementsByClassName('singleVezbe');
								console.log(exerciseContainer[exerciseContainer.length-1]);
								exerciseContainer[exerciseContainer.length-1].remove();
						});
				});
			</script>
		</form:form>
